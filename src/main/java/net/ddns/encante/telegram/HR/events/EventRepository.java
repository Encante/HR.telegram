...
public interface EventRepository extends JpaRepository<Event, Long> {
    ...
    @Query("SELECT e FROM Event e WHERE e.date BETWEEN ?1 - 604000L AND ?1 AND e.type = net.ddns.encante.telegram.HR.events.EventType.QUIZ_BAD_ANSWER")
    List<Event> findBadAnswerEventsFromLastWeek(Long currentDate);
    @Query("SELECT e FROM Event e WHERE e.date BETWEEN ?1 - 604000L AND ?1 AND e.type = net.ddns.encante.telegram.HR.events.EventType.QUIZ_GOOD_ANSWER")
